...
bool createFile(Map* user, char file_name[], bool file_protect[3], int file_length)
{
	...
	else
	{
		...
		//查找是否存在同名文件,若存在，则创建失败，提示用户并返回false;若不存在，则在该用户的文件列表结尾加入该文件
		while (op)
		{
...
bool deleteFile(Map*& user, char file_name[], AFD* afd)
{
	//新建一个File类型的变量file，指向打开的文件列表的头指针
	...
	while (file)
	{
		if (strcmp(file->file_name, file_name) == 0)
		{
			...
			{
				user->file = file->next;//若要删除的文件为列表的第一个文件，则指向其下一个文件
			}
			else
			{
				temp = prefile->next;
				prefile->next = temp->next;//若非列表的第一个文件，则令其前一个文件指向其后一个文件
			}
			return true;
		}
